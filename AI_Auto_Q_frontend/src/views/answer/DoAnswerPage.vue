...
const generateId = async () => {
  ...
  if (res.code === 0) {
    id.value = res.data as any;
  } else {
    message.error("获取唯一 id 失败，" + res.message);
  }
};
...
const loadData = async () => {
  ...
  if (res.code === 0) {
    ...
    message.error("获取应用失败，" + res.message);
    return;
  }
  ...
  if (res.code === 0) {
    ...
    if (!firstRecord) {
      ...
      message.warning("当前应用暂无题目");
      return;
    }

    questionContent.value = firstRecord.questionContent || [];
  } else {
    questionContent.value = [];
